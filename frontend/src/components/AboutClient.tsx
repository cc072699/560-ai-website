...
export function AboutClient({ about, site }: AboutClientProps) {
  ...
  const slides = [
    {
      category: '有硬实力的技术基因 / Tech DNA',
      title: '源自深圳的前沿算法沉淀',
      description: '五六零人工智能发源于深圳大湾区，传承领先的大语言模型、时序大数据预测及机器视觉底层核心算法能力。依托先进的算法工程化经验，确保我们的技术架构始终保持行业前列。',
      image: '/uploads/shenzhen_lab.jpg'
    },
    {
      category: '更懂实体痛点的本地方案 / Regional Delivery',
      title: '深耕广西的敏捷服务交付',
      description: '总部落地广西北海高新技术产业园。我们知晓小微科技企业的核心竞争力在于“交付与贴身服务”，因此建立了一支2小时内快速响应的本地技术交付队伍，切实消除传统实体数智化转型的后顾之忧。',
      image: about.imageUrl || '/uploads/1783560678234-41fljf97v.jpg'
    },
    {
      category: '协同共赢的开放体系 / Regional Ecosystem',
      title: '辐射东盟的产业普惠生态',
      description: '紧跟国家“中国-东盟信息港”战略，我们以北海为跨境合作桥头堡。通过定制开发中英双语、东盟多语种的智能工作流与高兼容物联网数据网关，积极联动政企、院校与行业主体，推动AI技术普惠共赢。',
      image: '/uploads/asean_connectivity.jpg'
    }
  ];
    ...
    <div className="flex flex-col w-full min-h-screen bg-[#FDFDFD] relative overflow-hidden pb-0">
      ...
      <section className="relative w-full h-[70vh] min-h-[580px] md:h-[78vh] md:min-h-[680px] flex items-center justify-center overflow-hidden z-10">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: `url('/uploads/about_hero_banner.jpg')`,
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-16 relative z-20 max-w-[1440px] text-left w-full pt-16 md:pt-24">
          ...
          <motion.p
            ...
            className="text-base md:text-lg lg:text-xl text-slate-200/90 max-w-2xl leading-relaxed font-normal mb-10 text-pretty"
          >
            让大模型及物联网智能感知能力切实落地，<br className="hidden md:inline" />
            服务广西，辐射东盟，助力区域人工智能产业协同普惠与共赢。
          </motion.p>
                  ...
                  {slides.map((_, index) => (
                    <button
                      ...
                      className={`h-3 rounded-full transition-all duration-300 ${
                        activeSlide === index ? 'w-8 bg-blue-600' : 'w-3 bg-slate-250 hover:bg-slate-350'
                      }`}
                    />
                  ))}
                </div>
      ...
      <section className="relative w-screen left-1/2 right-1/2 -ml-[50vw] bg-white border-t border-slate-200/40">
        ...
        {/* Flat full-screen container with NO rounded corners and NO padding (Expanded to h-[90vh] min-h-[850px]) */}
        <div className="relative w-full flex flex-col md:flex-row h-auto md:h-[90vh] min-h-[600px] md:min-h-[850px] overflow-hidden bg-slate-100">
          ...
          <div 
            ...
            className={`relative md:absolute md:inset-y-0 md:left-0 w-full md:w-[55%] flex flex-col justify-center items-center text-center p-8 md:p-16 z-10 transition-all duration-700 ease-in-out cursor-pointer md:[clip-path:polygon(0_0,100%_0,80%_100%,0_100%)] min-h-[350px] md:min-h-0 ${
              isLeftActive ? 'bg-[#0b5be8] text-white shadow-2xl' : 'bg-[#FAFAFA] text-slate-800'
            }`}
          >
            {/* Background image overlay */}
            <div 
              className={`absolute inset-0 bg-cover bg-center transition-all duration-700 ease-in-out ${
                isLeftActive ? 'mix-blend-luminosity opacity-50 scale-[1.03]' : 'mix-blend-normal opacity-[0.12] scale-100 grayscale'
              }`}
              style={{ backgroundImage: `url('/uploads/vision_bg.jpg')` }}
            />
            <div className={`absolute inset-0 bg-gradient-to-tr from-blue-900/60 via-transparent to-transparent pointer-events-none transition-opacity duration-700 ease-in-out ${
            ...
            <div className="relative z-20 flex flex-col items-center max-w-xl px-6 transition-all duration-700">
              <h3 className={`text-3xl md:text-6xl font-black mb-5 tracking-tight transition-all duration-700 ${
                isLeftActive ? 'text-white' : 'text-slate-900'
              }`}>生态构建</h3>
              
              <div className={`h-2 mb-8 transition-all duration-700 ${
                isLeftActive ? 'bg-white w-20' : 'bg-blue-600 w-12'
              }`} />
              
              <p className={`text-base md:text-2.5xl leading-relaxed font-bold transition-all duration-700 ${
                isLeftActive ? 'text-blue-50/95' : 'text-slate-500'
              }`}>
                推动AI技术全面普惠化与产业生态成熟化，构建协同共赢的人工智能产业生态。
              </p>
            </div>
          </div>
          
          {/* Right Column: Technology (60% width on desktop, absolute right, z-0. Left column always remains on top at z-10) */}
          <div 
            onMouseEnter={() => setHoveredSide('right')}
            onMouseLeave={() => setHoveredSide(null)}
            className={`relative md:absolute md:inset-y-0 md:right-0 w-full md:w-[60%] flex flex-col justify-center items-center text-center p-8 md:p-16 md:pl-36 z-0 transition-all duration-700 ease-in-out cursor-pointer min-h-[350px] md:min-h-0 ${
              isRightActive ? 'bg-[#0b5be8] text-white shadow-2xl' : 'bg-[#FAFAFA] text-slate-800'
            }`}
          >
            {/* Background image overlay */}
            <div 
              className={`absolute inset-0 bg-cover bg-center transition-all duration-700 ease-in-out ${
                isRightActive ? 'mix-blend-luminosity opacity-50 scale-[1.03]' : 'mix-blend-normal opacity-[0.12] scale-100 grayscale'
              }`}
              style={{ backgroundImage: `url('/uploads/mission_bg.jpg')` }}
            />
            <div className={`absolute inset-0 bg-gradient-to-tr from-blue-900/60 via-transparent to-transparent pointer-events-none transition-opacity duration-700 ease-in-out ${
              isRightActive ? 'opacity-100' : 'opacity-0'
            }`} />
            
            {/* Content container */}
            <div className="relative z-20 flex flex-col items-center max-w-xl px-6 transition-all duration-700">
              <h3 className={`text-3xl md:text-6xl font-black mb-5 tracking-tight transition-all duration-700 ${
                isRightActive ? 'text-white' : 'text-slate-900'
              }`}>技术赋能</h3>
              
              <div className={`h-2 mb-8 transition-all duration-700 ${
                isRightActive ? 'bg-white w-20' : 'bg-blue-600 w-12'
              }`} />
              
              <p className={`text-base md:text-2.5xl leading-relaxed font-bold transition-all duration-700 ${
                isRightActive ? 'text-blue-50/95' : 'text-slate-500'
              }`}>
                以“技术赋能+生态构建”为核心，助力传统企业高效完成数字化转型，推动区域人工智能产业规模化、生态化发展。
              </p>
            </div>
          </div>
      ...
      {/* 4. Brand Origin, Corporate Culture & Contact Us Combined (Longshine-style 1:1 Dark Forest Teal Theme) */}
      <section 
        ref={timelineRef}
      ...
      >
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-16 max-w-[1440px] relative z-10 w-full text-center">
          ...
          {/* Header Title Area (Ultra-High contrast white text) */}
          <div className="max-w-3xl mx-auto mb-28 text-center space-y-6">
            ...
            >
              ...
              <span className="text-slate-200 text-base md:text-xl mt-4 block font-medium leading-relaxed">
                其意是描绘科技与实业深度共生、智能普惠、绿色低碳的一派勃兴景象
              </span>
            </motion.p>
          ...
          <div className="relative max-w-5xl mx-auto mt-20 px-4 md:px-0">
            ...
            <div className="space-y-24 md:space-y-36">
              ...
              {/* Item 1: Brand Swatches (Left on desktop) */}
              <div className="relative flex flex-col md:flex-row md:justify-between items-start md:items-center w-full">
                ...
                >
                  <motion.div 
                    ...
                    transition={{ duration: 0.6 }}
                    className={`relative w-full rounded-[2.2rem] overflow-hidden p-8 text-left shadow-2xl border transition-all duration-500 group min-h-[280px] md:min-h-[340px] flex flex-col justify-between ${
                  ...
                  >
                    {/* Full Card Background Image (Crisp & High contrast) */}
                    <div className="absolute inset-0 z-0 select-none pointer-events-none overflow-hidden">
                      <img 
                        src="/uploads/brand_identity_bg.jpg" 
                    ...
                    <div className="relative z-10 flex flex-col justify-between h-full w-full">
                      ...
                      {/* Main text content */}
                      <div>
                        <span className="inline-block px-3 py-1 rounded-md bg-[#059669] text-xs font-black text-white tracking-widest uppercase mb-4 shadow-sm">
                          品牌标识
                        </span>
                        <h4 className="text-xl md:text-2.5xl font-black text-white mb-3 tracking-tight">科技之蓝，普惠之绿</h4>
                        <p className="text-slate-200 text-sm md:text-base leading-relaxed text-justify font-medium">
                          五六零人工智能以“560”为名，承载“五洲互联、六力聚能、零碳未来”的使命担当，深耕广西北部湾产业沃土，致力于用前沿AI大数据与物联网软硬件一体化交付方案，实实在在助推实体企业降本增效、完成绿色数智化转型。
                        </p>
                      </div>
                    </div>
                  </motion.div>
                ...
                {/* Center dot (Node 1) */}
                <div className={`absolute left-6 md:left-1/2 -translate-x-1/2 w-9 h-9 rounded-full border transition-all duration-500 flex items-center justify-center text-xs font-bold z-20 select-none ${
                  activeTimelineIndex === 0 
              ...
              {/* Item 2: Core Values (Right on desktop) */}
              <div className="relative flex flex-col md:flex-row md:justify-between items-start md:items-center w-full">
                {/* Left empty space */}
                <div className="hidden md:block w-[46%]" />
                ...
                >
                  ...
                  >
                    {/* Full Card Background Image (Values Handshake - Vivid & Crisp) */}
                    <div className="absolute inset-0 z-0 select-none pointer-events-none overflow-hidden">
                      <img 
                        src="/uploads/values_handshake.jpg" 
                    ...
                    <div className="relative z-10 flex flex-col justify-end h-full w-full">
                      <div>
                        <span className="inline-block px-3 py-1 rounded-md bg-[#E25C34] text-xs font-black text-white tracking-widest uppercase mb-4 shadow-sm">
                          价值观
                        </span>
                        <h4 className="text-xl md:text-2.5xl font-black text-white mb-3 tracking-tight leading-tight">
                          好技术，重交付，真诚利他
                        </h4>
                        <p className="text-slate-100 text-sm md:text-base leading-relaxed text-justify font-medium">
                          我们坚信，只有为客户创造价值的科技才有生命力。公司拒绝花哨的PPT概念，坚持“交付与贴身服务”重于一切。通过广西北海本地的快速响应团队，切实解决小微与传统企业智能化转型中交付困难的痛点。
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              {/* Item 3: Mission (Left on desktop) */}
              <div className="relative flex flex-col md:flex-row md:justify-between items-start md:items-center w-full">
                {/* Left Card */}
                <div 
                ...
                >
                  ...
                  >
                    {/* Full Card Background Image (Mission Climber - Vivid & Crisp) */}
                    <div className="absolute inset-0 z-0 select-none pointer-events-none overflow-hidden">
                      <img 
                        src="/uploads/mission_climber.jpg" 
                        className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-700" 
                        alt="企业使命背景"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-955 via-slate-950/80 to-transparent z-10" />
                    ...
                    <div className="relative z-10 flex flex-col justify-end h-full w-full">
                      <div>
                        <span className="inline-block px-3 py-1 rounded-md bg-[#C08E32] text-xs font-black text-white tracking-widest uppercase mb-4 shadow-sm">
                          企业使命
                        </span>
                        <h4 className="text-xl md:text-2.5xl font-black text-white mb-3 tracking-tight leading-tight">
                          让领先算法回归产业实效
                        ...
                        <p className="text-slate-100 text-sm md:text-base leading-relaxed text-justify font-medium">
                          五六零人工智能旨在打破昂贵的人工智能技术壁垒。我们致力于打造高性价比、易部署、易运营的定制大模型与自动化工作流，让科技回归实体产业，使中小实体微企都能平等地享受算法红利。
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              {/* Item 4: Vision (Right on desktop) */}
              <div className="relative flex flex-col md:flex-row md:justify-between items-start md:items-center w-full">
                {/* Left empty space */}
                <div className="hidden md:block w-[46%]" />
                ...
                >
                  ...
                  >
                    {/* Full Card Background Image (Vision Connectivity - Vivid & Crisp) */}
                    <div className="absolute inset-0 z-0 select-none pointer-events-none overflow-hidden">
                      <img 
                        src="/uploads/vision_connectivity.jpg" 
                        className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-700" 
                        alt="企业愿景背景"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-955 via-slate-950/80 to-transparent z-10" />
                    ...
                    <div className="relative z-10 flex flex-col justify-end h-full w-full">
                      <div>
                        <span className="inline-block px-3 py-1 rounded-md bg-[#2563EB] text-xs font-black text-white tracking-widest uppercase mb-4 shadow-sm">
                          企业愿景
                        </span>
                        <h4 className="text-xl md:text-2.5xl font-black text-white mb-3 tracking-tight leading-tight">
                          共筑区域数字智能新生态
                        ...
                        <p className="text-slate-100 text-sm md:text-base leading-relaxed text-justify font-medium">
                          立足广西北海，面向东盟，我们将深耕多语种智能大模型和物联网数据感知交付方案。积极构建多方协同共赢的普惠AI生态，成为中国-东盟区域最值得信赖、最懂实体转型痛点的数字智能化伙伴。
                        </p>
                      </div>
                    </div>
                  </motion.div>
        ...
        {/* Divider line before Contact */}
          ...
          {/* Glowing geometric glassmorphism backdrops on the bottom right (Sharing the same timeline background) */}
          ...
          {/* Contact Section Content (Sharing the same background) */}
